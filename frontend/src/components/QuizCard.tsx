...
export function QuizCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute -top-12 -right-12 w-48 h-48 rounded-full ${style.badgeBg} opacity-40 blur-2xl group-hover:scale-110 transition-transform duration-700`}
      />
      <div
        className={`absolute top-1/2 left-0 w-32 h-32 rounded-full ${style.badgeBg} opacity-20 blur-xl group-hover:translate-x-4 transition-transform duration-1000`}
      />
      ...
      <div className={`relative z-10 ${style.text} w-full`}>
        {category && (
          <div className="flex items-center gap-3 mb-4">
            <span
              className={`px-3 py-1 rounded-full bg-white/40 backdrop-blur-sm text-[10px] font-bold uppercase tracking-widest ${style.text}`}
            >
              {category}
            </span>
          </div>
        ...
        <h2 className="font-display text-xl lg:text-2xl font-extrabold mb-4 leading-tight line-clamp-3">
          {title}
        </h2>
